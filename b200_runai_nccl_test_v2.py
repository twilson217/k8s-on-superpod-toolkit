#!/usr/bin/env python3
"""
Script to run multi-node NCCL tests in a RunAI environment (v2 - minimal configuration).

This version uses NCCL defaults and only sets debug environment variables when --debug is enabled.
...
Usage:
    python b200_runai_nccl_test_v2.py --project <PROJECT_NAME> --nodes <NUM_NODES> [--debug]

Example:
    python b200_runai_nccl_test_v2.py --project test --nodes 2
    python b200_runai_nccl_test_v2.py --project test --nodes 2 --debug
"""
...
def run_nccl_test(project_name, num_nodes, debug=False):
    ...
    if debug:
        print("Debug mode: ENABLED (NCCL_DEBUG=INFO, NCCL_DEBUG_SUBSYS=INIT,NET)")
    else:
        print("Debug mode: DISABLED (using NCCL defaults)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run multi-node NCCL tests in RunAI environment (v2 - minimal configuration)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Run NCCL test with default configuration (no custom env vars)
  python b200_runai_nccl_test_v2.py --project test --nodes 2
...
Note:
  This v2 version uses NCCL default settings and only enables debug
  environment variables (NCCL_DEBUG, NCCL_DEBUG_SUBSYS) when --debug is specified.
        """
    )
